...
- **Trap: a Loose piece is still its anchor's child** (`TopLevel`, so it flies in the world), so it
  dies with its anchor. That is accepted for the chunk; an effect that must outlive its hull rides
  `Fx.World` instead.
