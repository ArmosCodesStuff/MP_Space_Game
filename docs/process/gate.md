...
- Every change carries its checks: they would fail without the change; 3 varied situations; no knife
  ...
  the old-truth checks; visible changes have named frames; wire changes have guest-role checks; every
  class ability or drive row has at least three distinct checks (owner).
...
`pass`; `defects` (behaviour, authority, lifetime, a missing or loosened check, a wrong number, an
unhashed table: each with file:line and the exact replacement text) and `notes` (comments, prose, names,
record lines: same form). Only defects fail the gate. A gate 2 reviews ONLY `git diff` of the fix commits
against the defects it was given, in at most 40 lines; it never re-reviews what gate 1 passed. There is no
gate 3.
