...
Three sections in this order, the head under 40 lines:
1. **Decisions**: numbered, one line each (the gate reads only this and the diff).
2. **Jobs**: the list, foundations first, a status mark per job (PRE / POST / commit / files); then the
   ...
   never accumulated); id ranges the plan allocated; the newest COORDINATOR NOTE number applied.
...
where a check must read a table, prove the table against literals in one place. A new or rewritten check
passes on two seeds in the test phase; a flaky one is fixed at rung 3 until three seeds pass. Mutants only
for authority, save-format and removed-path invariants, and only when a check's wiring is in doubt.
`verify.ps1` refuses a `scripts/` change with no harness change since the last `VERIFIED:` commit. Every
lane call in a role body is wrapped in the catch-and-continue helper so one throw cannot hide the rest.
...
`symptom -> cause -> where` for `docs/TRAPS.md`. Lanes do not edit `CHANGES.md`, `DESIGN.md` or
...
`status` (done / stopped_context / red / blocked), `head`, at most 10 lines, `open`. Red only if
typecheck or quick stays red after two honest attempts, with the failing lines in `open`.
