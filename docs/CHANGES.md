...
**Latest change:** a guest's gear levels travel with its identity, so a host lifts its parts by ITS
levels, not the host's, and a level bought refits the ship at once, offline too (`Equipment.SanitizeLevels`,
`PlayerShip.Levels`; the session fingerprint changed). Before it: a turret left standing -- a
