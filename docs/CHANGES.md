...
**Latest change:** the launcher -- `scripts/Builds.cs` (an installed build that can be replaced
from a source of truth), a separate `launcher/` program with no decisions of its own, and
`tools/pack.ps1`, the one command that makes a release. `powershell -File tools\pack.ps1`; the five
owner steps are in `docs/README.md` under Releasing.
