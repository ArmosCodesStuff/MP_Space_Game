...
the owner's drawings, bombers that park on the carrier's deck and lift off and land like the hauler, and
a blue-and-gold default (see Unreleased, the first section). The owner asked for **one test run at the
end of each batch and no mutant runs** for the rest of that session.

**State as of 2026-09-22: the owner's SECOND batch was built in three slices, each verified and
committed on its own, and all three are done** -- the battleship's broadside, a third class (the
destroyer), faster ships, closer bombers; four outposts the escort visits and offloads at, uncapped
half-hull raider waves, a tougher hauler, the owner's fighter report; the owner's Lancer report (the
beam a hard lock, the charge on the pin) on a new base for more bosses; and a second boss, the Drake
Bastion, on the even levels (see Unreleased). On top of the first batch -- boss-drop gear,
...
**Latest change:** the owner's line art, the carrier's deck, and blue and gold (above).
...
hull and accent colours (blue and gold by default). The **destroyer**, the fastest (130 u/s), aims two main
...
off in turn from its deck and landing back on it -- and has activated point defence. The **TIO** offers boss missions -- the Silver Lancer on the odd levels, the Drake Bastion on the even
...
- **Sizes follow the drawings.** The battleship keeps its 224 u and so is slender; its hit capsule is the
  drawn hull, a 13 u half-beam where it was 40 -- a much narrower target for anything aimed at the hull
  (the Drake's shells and scrap, the rock, a heavy's missile). The destroyer is 130 u (from 201.6) to be
  the smallest of the three, as asked, with a 17 u half-beam. Say if either should be larger.
- **Turrets wear the accent colour** (gold by default), not the hull's -- the accent was already named
...
- **Bombers take off on their own 0.83 s clock**, beside the fighters' rather than sharing it, and land
  without a queue (they come home about 0.83 s apart anyway). Parked at 40% -- the size that fits the
  carrier's white deck between the runway and the hull.
...
### The owner's line art, the carrier's deck, blue and gold, the escort's threat, boss sounds, damage numbers (2026-09-22, in the WarShips_Version_L fork)
...
- **Every ship is the owner's line art now**: grey on transparent for the game to tint, each made exactly
  ...
  - **Carrier** (170 u): a runway down its centre with white deck either side, three sponsons a flank;
    point defence on the two middle sponsons and the stern block.
  - **Battleship** (224 u, slender: a 13 u half-beam): its four painted turrets painted over, the four
    moving main turrets standing where they stood on the spine; point defence on the stern quarters.
  - **Destroyer** (130 u, the smallest of the three; a 17 u half-beam): main turrets on the fore spine
    and the central plate, point defence on the stern quarters, the turrets at 0.8x the battleship's.
...
- **Bombers park on the carrier's deck**, small (40%), in bays either side of the runway, and **take off
...
- **The default hull colour is blue** (0.30, 0.50, 0.95), the accent gold: the line art is white until
  tinted, and the old pale-blue default left it near white. A pilot still in that pale default loads in
  the blue.
...
- **A raider that leaves the world is dropped as every ship's target** (`PlayerShip.Forget`, from
  `Hub.DropRaider`): the wing's order, the bombers' strike and each turret's aim. A withdrawn hunter
