...
**State as of 2026-09-21: plug and play for any network, a real build handshake, and a full audit
of every script (see Unreleased, first section).** The last VERIFIED commit is the baseline; the map
of every script, member, RPC, spawn site, event hookup and asset is `version/MAP.md`
(`python tools/map.py`, regenerated by `verify.ps1 -Update`). **Next: the owner's feature batch**
(gear, loot, escort, reconnection, tutorial -- see Next up; the answered design questions are
there too).
...
unused, the smoke test three full runs in a row (**524 pass, 6 of 6 runs** each: solo, a host with
...
screenshot sweep (**67 frames, 0 lint**) on the developer's own GPU with the project's Forward+
...
a failed mission brings the boss's raiders to your base. The **base** (B) runs an idle economy --
...
### Next up — the owner's batch of 2026-09-21 (design questions answered; build in this order)

1. **Foundations:** the save fields (inventory, loot, tutorial-seen, reconnection memory), the gear
   tables, a mission framework the escort can share with the boss. Bump `Game.Version` (the owner's
   rule: older characters grey out).
2. **Gear: 12 items per slot type** = 4 specialisations × 3 rarities, each leaning hard one way (a
   carrier bay: +200% fighter speed and damage, +50% turning, only 2 fighters; another: 9 fighters,
   22% slower and weaker; one between). Weapon and Utility per class; Engines, Shield, Hull general
   (filtered only by capital ship or not); 12 chips. Rarities: Common / Rare / Epic -- L1-5 drop
   Common, L6-10 add Rare, L11+ add Epic (retire the unused Uncommon and Legendary).
3. **Loot:** bosses only, crates per pilot, visible only to that pilot, flown over to collect; saved
   in batches 2-3 s after the LAST pickup (the timer resets per item), to the pilot's own file. Guests
   get their own EXP and loot.
4. **The hauler:** DISPATCH offers an opt-in escort. Plain dispatch = a transport with a % chance to
   fail (lose everything) after the portal, from a new EVASION upgrade. The escort mission = 5x the
   income: the hauler flies a long route across the map while raider waves hunt it; it must reach the
   portal alive. Auto-sell is locked until the level-3 boss is beaten AND an upgrade is bought costing
   3x the average level-5 cost across the salvager upgrades (estimate once, hard-code).
5. **Reconnection:** after an unexpected drop, retry the same host 3x over ~20 s, then a RECONNECT
   button; the host holds the pilot's place (party slot, READY, arena) for 90 s.
6. **Tutorial:** corner hints the first time each system is met; fade in ~8 s; never take input; off
   in the Esc menu; remembered per character.
- Still: `Hub.BeginPlacement` for the first non-instant ability; wormhole transit into an instanced
  system; balance (the battleship out-damages the carrier about 2.4× on paper; levers in `DESIGN.md`).
