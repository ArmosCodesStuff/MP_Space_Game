...
**Latest change:** the sky is three parallax layers instead of one static image, so flying reads
as flying (`scripts/Sky.cs`, a row per layer). Before it: the launcher -- `scripts/Builds.cs`, a
separate `launcher/` program with no decisions of its own, and `tools/pack.ps1`, the one command
that makes a release. The five owner steps are in `docs/README.md` under Releasing.
...
*(Items 11 and 12 -- the gear-line clarity pass and the dumb launcher -- are built; see
Unreleased. Nothing is outstanding from the batch of 2026-09-23.)*
...
**`scripts/Builds.cs` is new, and it is the mechanism, not the launcher.** An installed build is a
set of files with known hashes; a release is the same set with different hashes; updating is
replacing only the files whose hashes differ, through a temp name and a verify, and never touching
anything the release does not name. `BuildPart` rows (id, asset, sha, bytes, paths), `BuildOutcome`
as the decision table in one enum, and `IBuildSource` as the seam -- a folder for the checks, HTTPS
for the launcher, so the game never carries `System.Net.Http`. **No Godot type in the file**, which
is what lets the launcher link the same source the game compiles.

**`launcher/` is a separate net8.0-windows program** with a changelog box, a status line, a
progress bar, UPDATE and PLAY -- and no decisions of its own: every branch is a `Builds.Decide`
result switched onto two Enabled flags and one label. That is what lets it stay frozen once
shipped, and it is why the rung-3 checks are enough. Self-contained and compressed, 66 MB, so a
player needs no .NET runtime installed first. It is NOT in `Warships.sln`: `verify.ps1` builds the
solution, and the launcher's warnings do not belong in the game's gate.

**It reads `https://github.com/<owner>/<repo>/releases/latest/download/<asset>`** -- the fixed
redirect, not the REST API, so there is no token, no 60-per-hour rate limit and no JSON schema
anyone else owns. A `source.txt` beside the exe overrides that base URL, which is how a test
install points at a local folder and how the host moves without a new launcher. **The download is
public on purpose:** anything the launcher can fetch unaided a player can fetch unaided, so a token
or a key inside a file players hold is not a secret; a real gate would need a service in front of
the download.
...
headlessly, publishes the launcher, hashes all 189 files, cuts `NOTES.txt` out of this file, zips
the two parts and prints the upload list. Measured: **189 files, 191 MB installed; an ordinary
release changes 5 of them, 10 MB zipped against 73.** The part split is decided in the script and
written out as 189 explicit `in=` lines, because pattern matching is logic and logic that might
need fixing belongs on the repo side of the line, never in a program that must stay frozen.

**Safety, by geometry rather than by care.** `user://` is `%APPDATA%\Godot\app_userdata\Warships\`
-- outside the install, so the launcher's write scope cannot reach a save. Every path goes through
`Builds.Resolve`, which refuses anything that is not a plain relative path inside the install root.
The delete set is empty by construction: a file is only ever renamed over, and the only deletes in
...
pack checks the line count against the file count. The launcher's `AssemblyName` was `Warships`,
which is the game's own exe name in the folder the launcher sits in. And self-contained WinForms is
145 MB until `EnableCompressionInSingleFile` makes it 66.
