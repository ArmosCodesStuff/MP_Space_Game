...
committed on its own. All three are done** -- the battleship's broadside, a third class (the
...
Bastion, on the even levels (see Unreleased, the first four sections). Nothing of the batch is left. On top of the first batch -- boss-drop gear,
...
**Latest change:** slice 3b, the Drake Bastion (above).
...
unused, the smoke test three full runs in a row (**734 pass, 6 of 6 runs** each: solo, a host with
...
screenshot sweep (**84 frames, 0 lint**) on the developer's own GPU with the project's Forward+
...
point defence. The **TIO** offers boss missions -- the Silver Lancer on the odd levels, the Drake Bastion on the even
...
- **A guest that rejoins the arena during a Drake's throw sees no rock and no lane, yet the rock can
  still hit it.** A boss's events (its telegraphs, the rock) are sent once, to the pilots already in the
  arena; the Lancer's beam telegraph has the same gap for a rejoining guest. A catch-up for the boss's
  events in progress (`Hub.NetMySector`) is the fix, and needs the arena pair's drop reshaped to test it.
