...
**Latest change:** the lanes -- four courier routes from the base to its outposts, each carrying a
quarter of the passive income, a blockade that cuts one, and an upgradable gun at every outpost to
answer it. The predicted missile is a row of `Missiles.All` now, because there are two launchers.
...
12. **A DUMB LAUNCHER, and clients that update themselves.** A second, tiny executable: the
    ...
    the whole join UI, and a second copy of that is two UIs to keep in step.
    - It compares per-file hashes against the release's manifest and replaces only what changed.
      **`version/MANIFEST.sha256` already is that manifest** -- 217 files, regenerated by the bar --
      so the part people usually have to build exists.
    - This is a C# game: the managed assembly sits BESIDE the executable rather than inside the
      .pck, so an update replaces a folder of files, not one file. Same loop, more entries.
    - The launcher exists so an update can land while the game is not running -- a running
      executable cannot overwrite itself -- and so a build that bricks itself can still be fixed.
    - Two open costs, neither of them code: an unsigned pair of executables means two SmartScreen
      prompts (a certificate is about $100-300 a year), and the launcher must stay frozen.
    - The release notes are `docs/CHANGES.md`, which is kept properly, so the changelog is free.
