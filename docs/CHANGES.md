...
committed on its own. Slices 1, 2 and 3a are done** -- the battleship's broadside, a third class
(the destroyer), faster ships, closer bombers; four outposts the escort visits and offloads at,
uncapped half-hull raider waves, a tougher hauler, the owner's fighter report; and the owner's Lancer
report (the beam a hard lock, the charge on the pin) on a new base for more bosses (see Unreleased,
first three sections). **Slice 3b is next** (see Next up: the Drake Bastion). On top of the first batch -- boss-drop gear,
...
unused, the smoke test three full runs in a row (**724 pass, 6 of 6 runs** each: solo, a host with
...
**Slice 3b -- the Drake Bastion** (3a, the Lancer and the base for more bosses, is done):
- **Drake Bastion** (base hull 700, sprite `1.webp` -- recovered from the session transcript, not yet
  in the repo; ~420 u): a slow,
...
- **Odd mission levels are the Lancer, even levels the Drake**, one ladder: add the Drake to
  `Missions.Bosses` and the ladder alternates by itself.
- **The Drake stays immobile during its specials**, as the Lancer now does.
