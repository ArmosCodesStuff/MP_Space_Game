...
**Latest change:** slice S1 of the multiplayer redesign -- a guest is online only once the host's
welcome arrives (`Net.NetWelcome`, `Net.Admitted`), a silent peer is kept 8 s and ENet's throttle
never falls (`Net.Link`), a guest pressing HOST lets its goodbye out, and a typed address that never
answers says what an address is for (Unreleased, first section; compiled, engine rungs still to
run). Before it: a guest's gear levels travel with its identity, so a host lifts its parts by ITS
levels, not the host's, and a level bought refits the ship at once, offline too (`Equipment.SanitizeLevels`,
`PlayerShip.Levels`; the session fingerprint changed). Before that: a turret left standing -- a
freighter's -- shoots anything hostile, missiles and small craft first, and a practice dummy only
while nothing that can die is in reach (`Targeting.Sentry`); and no gun holds a target that has
left the world (`Turret.StillThere`). Then: stealth stops choosing, never hitting -- a filter answers
`Chooses` or `Hits`; a boss whose only pilot is dark closes on and aims at where it last saw anyone,
and its clocks run on; a ring, a beam, a ram or a raid's burst lands on a dark ship
(`TargetFilter.Chooses`/`Hits`, `Raider.Reachable`). Earlier: every rate buff reaches the guns and
a speed lift reaches the throttle; rate buffs add (`PlayerShip.Cadence`, `Stat.With`); Gunnery and Cooling
points shorten what they say they shorten; before that the sky's direction and its far-out edge,
and the Drake warping back before it throws (`scripts/Sky.cs`, `scripts/Drake.cs`); and before
that the launcher was deleted -- `PLAY.bat` -> `play.ps1` -> `tools/install.ps1` puts the
published build on a machine with nothing installed, and `tools/pack.ps1` is the one command that
makes a release. The five owner steps are in `docs/README.md` under Releasing. Housekeeping that
changed nothing the game does -- art nothing loads and 45 translations Godot made of `version/`
taken out of the release, a record that no longer describes the launcher as current, a `text`
step in `verify.ps1` -- is under Unreleased, below the sky entry.
...
- **A chip cannot be levelled.** The equipment window's chip rows show UNEQUIP or nothing, never
  UPGRADE (`EquipmentWindow.cs:54`, `off ?? Upgrade(...)`): a fitted chip always has the UNEQUIP
