...
**State as of 2026-09-21: the owner's feature batch is built -- boss-drop gear (96 parts), per-pilot
loot, the hauler's escort and lone runs, reconnection, and the tutorial's corner hints (see
Unreleased, first section)**, on top of plug and play for any network and the full audit (second
section). Save format 2: characters from the last release are greyed out. The last VERIFIED commit
is the baseline; the map of every script, member, RPC, spawn site, event hookup and asset is
`version/MAP.md` (`python tools/map.py`, regenerated by `verify.ps1 -Update`). The guesses this
batch made where the owner's answers left a gap are under Open questions -- check those first.
...
gear** -- 96 parts that each lean hard one way, fitted from the pilot's **hold** in the equipment
...
- **Pickup is contact only** (the owner's "collected by flying over them"): no magnet. The crates lie
  on a 240 u ring where the boss died; the victory window is **20 s**, counted down on the boss bar.
...
- **Loot** (`Loot.cs`): the host rolls each pilot's drops at a boss kill -- 2 crates, 3 from boss
  ...
  **A 20 s victory window** is counted down on the boss bar; the boss's escorts and raiders die with
  it. Crates not flown over reach the hold the next time the pilot enters a world. The radar shows
...
- **Found by looking at the sweep and by the `-Wan` run**, and fixed: **the equipment window (I)
  ...
  1080 screen; **a boss that dies takes its warnings with it** -- with the 20 s victory window, a beam
...
- Nothing known. Not yet done: a session between two homes (see Next up), and flying the new gear
  for balance.
