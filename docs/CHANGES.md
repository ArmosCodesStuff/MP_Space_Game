...
**State as of 2026-09-22: the owner's SECOND batch is being built in three slices, each verified and
committed on its own. Slice 1 is done** -- the battleship's broadside, a third class (the destroyer,
with the missile racks and a missile burst), faster carrier and destroyer, bombers that close in (see
Unreleased, first section). **Slices 2 and 3 are next** (see Next up: the hauler's outposts and the
Drake Bastion), with two reports from the owner folded in. On top of the first batch -- boss-drop gear,
per-pilot loot, the hauler's runs, reconnection, the tutorial -- and plug and play for any network.
Save format 2: characters from the last release are greyed out; pilots from the first batch load, their
battleship's missile racks carried over to the destroyer. The last VERIFIED commit is the baseline;
the map of every script, member, RPC, spawn site, event hookup and asset is `version/MAP.md`
(`python tools/map.py`, regenerated by `verify.ps1 -Update`). The guesses made where the owner's
answers left a gap are under Open questions -- check those first.
**Latest change:** slice 1 (above). Before it: a won arena is left by every pilot pressing
**RETURN** rather than on a clock.
...
unused, the smoke test three full runs in a row (**705 pass, 6 of 6 runs** each: solo, a host with
...
screenshot sweep (**79 frames, 0 lint**) on the developer's own GPU with the project's Forward+
...
**handle like naval ships** — no strafing, a turning radius. Three of nine planned classes are
...
cooldown), and has activated point defence. The **destroyer**, the fastest (130 u/s), aims two main
...
point defence. The **TIO** offers boss missions: everyone READY opens a portal to the boss's arena;
...
an **ESCORT** (the long way round, hunted by raider waves, 5x the pay); AUTO-SELL waits for the
...
| Battleship | | Destroyer | | Carrier | |
|---|---|---|---|---|---|
| Space (hold) | main guns | Space (hold) | main guns | Space | fighters: attack the target |
| G | salvo ↔ staggered | G | salvo ↔ staggered | R | fighters: recall |
| F | broadside (3 volleys, 10 s cooldown) | F | missile burst (magazine of 2 bursts) | F | bomber strike (unguided torpedoes) |
| Q | point defence (15 s window, 15 s recharge) | R | reload missiles (16 s) | Q | point defence (15 s window, 15 s recharge) |
| | | Q | point defence | | |
...
**Slice 2 -- the hauler's escort, as the owner specified (2026-09-22):**
- Four **outposts**, permanent small stations 2000 u out on the diagonals, labelled OUTPOST SE / NE /
  NW / SW and shown on the radar (candidate art: `art_unused/outpost.png`).
- The escort route runs **counter-clockwise from the south-east: SE → NE → NW → SW → the portal**, and
  the hauler **stops 4 s at each outpost** with an **offloading animation: the loading animation in
  reverse**.
- A raider wave **every 20 s for the whole route**; pay stays ×5. **The first wave is light fighters
  only; every other wave includes a heavy.** Escort-wave lights and heavies have **half HP**. **Hauler
  hull +75%.**
- **Owner's report: the carrier's fighters sometimes fly through an escort's light raiders without
  firing.** Likely cause, not yet proved: `Wing.TickFighter` starts a run only within 300 u AND 12° of the
  nose, and a small fast raider can sit inside the fighter's ~100 u turning circle so it never lines up.
  Reproduce with a check first (fighters sent at an escort light must land hits), then fix.

**Slice 3 -- the bosses:**
- **Silver Lancer** base hull 760. **Drake Bastion** (base hull 700, sprite `1.webp`, ~420 u): a slow,
  dodgeable projectile main gun (~2.4 DPS); on the trident's 15 s slot a warp to range then a **scrap
  shotgun** of 7 debris in a fixed fan; on the beam's 30 s slot a **tractor-beam asteroid throw** (a large
  asteroid, `2.png`/`3.png`, hurled along a red telegraphed lane, slow to start then very fast, breaking
  on impact); its specials 1.25× the Lancer's damage with ~25% longer telegraphs; no escorts.
- **Odd mission levels are the Lancer, even levels the Drake**, one ladder.
- **Both bosses stay immobile during their specials** (the Lancer's ram still moves). **Owner's report:
  the Lancer turns while its beam charges, and can start charging before its escorts have pinned the
  pilot.** Plan: from the moment the beam special starts the boss holds position; it may turn to face
  the pilot only BEFORE the charge; the charge starts when the web has actually pinned the pilot (or,
  as a fallback, the escorts are dead or overdue); from the charge to the end of the beam it is a HARD
  lock -- no turn, no move -- checked with the boss aimed away and the pilot moving, and on a guest.
- The intermittent arena-host `NullReferenceException` (Known broken).
...
- Balance with gear in play: the battleship out-damages the carrier about 2.4x on paper, and the
