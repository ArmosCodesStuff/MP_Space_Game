...
**Latest change:** a pilot shot down is back in 24 s (the owner's ruling, `PlayerShip.StasisTime`), and
the pirate base's launcher holds its aim on the last point it saw a pilot gone dark. Before it: the
pirate base has no guns -- a siege's damage is its garrison's, plus one CRUISE
MISSILE every 15 s from the base (a red lane 1.5 s ahead, riding the base; 120 u/s, 4500 u, 126
damage, 30 hull at level 1) that any weapon a pilot carries can shoot down (`Tag.Hulled`,
`Shots.Cruise`). Before it: the EQUIPMENT BASE -- a pad right of the station with the recycler on it,
where a part is levelled or scrapped only 10 s clear of combat (`scripts/Landmarks.cs`, a row per
place at home, which also says which docks each station carries). Before that: every beam the game
fires is its own note, and every beam is a quarter quieter -- a row of `Beam.All` (`scripts/Beam.cs`)
names a flash's colour, its file and its pitch, and every file a beam is heard by is stored at 0.75 of
where it shipped; a sniper's railgun is heard on guests now. And before that: the lanes' couriers dock
nose-in at the base's service arms and at a clamp on each outpost, instead of turning round in open
space (`scripts/Docks.cs`, a row per dock). Everything earlier in this batch -- a guest's gear levels
travelling with its identity, a freighter's turret shooting anything hostile (`Targeting.Sentry`),
stealth that stops choosing but never hitting, rate buffs that reach the guns, the sky and the
Drake, the launcher's removal, and housekeeping that changed nothing the game does -- is under
Unreleased, newest first. `PLAY.bat` -> `play.ps1` -> `tools/install.ps1` puts the published build
on a machine with nothing installed, and `tools/pack.ps1` is the one command that makes a release;
the five owner steps are in `docs/README.md` under Releasing.
