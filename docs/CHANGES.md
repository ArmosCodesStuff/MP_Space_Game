...
committed on its own. Slices 1 and 2 are done** -- the battleship's broadside, a third class (the
destroyer), faster ships, closer bombers; then four outposts the escort visits and offloads at,
uncapped half-hull raider waves, a tougher hauler, and the owner's fighter report fixed (see
Unreleased, first two sections). **Slice 3 is next** (see Next up: the Drake Bastion, and the Lancer's
beam as a hard lock). On top of the first batch -- boss-drop gear,
...
unused, the smoke test three full runs in a row (**718 pass, 6 of 6 runs** each: solo, a host with
