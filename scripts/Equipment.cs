...
// missiles in the magazine). The host applies every pilot's gear (it resolves damage and hull):
...
public static class Equipment
{
    ...
    // every weapon's damage on a class
    ...
    // every weapon's damage on EITHER class, for a general part
    private static (string, double)[] AllDamage(double p) =>
        DamageStats(ShipClass.Battleship).Concat(DamageStats(ShipClass.Carrier)).Select(s => (s, p)).ToArray();
    ...
    private static IEnumerable<ItemDef> Build()
    {
        ...
        yield return Kit("bs_missile_rack", "Missile Rack", GearSlot.Utility, ShipClass.Battleship, "the missile and its magazine");
        ...
        var bs = ShipClass.Battleship; var cv = ShipClass.Carrier;
        IEnumerable<ItemDef>[] lines =
        {
            ...
            // ── battleship missile rack ──
            Line("bs_salvo", "Salvo Rack", GearSlot.Utility, bs, "more missiles a magazine, a slower reload",
                 None, new[] { ("missile_reload", -0.25), ("missile_damage", -0.20) },
                 new[] { ("missile_mag", new[] { 1, 2, 3 }) }),
            Line("bs_buster", "Buster Rack", GearSlot.Utility, bs, "a devastating missile, slow and clumsy",
                 new[] { ("missile_damage", 1.00) }, new[] { ("missile_speed", -0.20), ("missile_turn", -0.30) }),
            Line("bs_seeker", "Seeker Rack", GearSlot.Utility, bs, "missiles that hunt; lighter warheads",
                 new[] { ("missile_turn", 1.00), ("missile_speed", 0.20) }, new[] { ("missile_damage", -0.20) }),
            Line("bs_loader", "Loader Rack", GearSlot.Utility, bs, "reloads fast, lighter warheads",
    ...
    // wrong slot or for the other class changes nothing: the shares, and the whole additions.
