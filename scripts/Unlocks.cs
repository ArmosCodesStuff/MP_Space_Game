...
// (Opens) and which one of those it is (Nth: ability 2, chip slot 5). Nothing else names a level:
// every question about a wall comes through the queries below.
// ─────────────────────────────────────────────────────────────────────────────
public enum Measure { Pilot }                       // the pilot's highest level reached (Character.Peak)
public enum Opens { Ability, ChipSlot }
...
public readonly struct Unlock
{
    ...
    public Unlock(Measure by, int at, Opens what, int nth) { By = by; At = at; What = what; Nth = nth; }
}
